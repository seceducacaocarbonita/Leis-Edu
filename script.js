...
function attachEvents() {
  ...
  els.featuredGrid.addEventListener("click", handleFeatureGridEvent);
  els.favoritesGrid.addEventListener("click", handleFeatureGridEvent);
  els.recentGrid.addEventListener("click", handleFeatureGridEvent);
...
/**
 * Renderiza um gráfico de rosca (donut) genérico.
 * @param {object} config
 * @param {HTMLElement} config.chartEl       - Elemento do gráfico SVG/CSS
 * @param {HTMLElement} config.totalEl       - Elemento que exibe o total
 * @param {HTMLElement} config.legendEl      - Elemento da legenda
 * @param {HTMLElement|null} config.summaryEl - Elemento do resumo textual (opcional)
 * @param {Array<{label:string,count:number}>} config.groups - Dados agrupados
 * @param {string[]} config.palette          - Paleta de cores
 * @param {string} config.unitLabel          - "aluno(s)" | "registro(s)"
 * @param {string} config.emptyMessage       - Mensagem quando não há dados
 * @param {string} config.summaryText        - Texto do resumo quando há dados (recebe groups.length)
 * @param {string} config.emptySummaryText   - Texto do resumo quando não há dados
 */
function renderDonutChart({
  chartEl, totalEl, legendEl, summaryEl,
  groups, palette, unitLabel,
  emptyMessage, summaryText, emptySummaryText
}) {
  ...
  if (summaryEl) {
    summaryEl.textContent = total ? summaryText(groups.length) : emptySummaryText;
  }

  if (!groups.length || total <= 0) {
    chartEl.classList.add("is-empty");
    chartEl.style.background = "conic-gradient(var(--border) 0deg 360deg)";
    legendEl.innerHTML = `<div class="student-donut-empty">${emptyMessage}</div>`;
    return;
  }
  ...
  legendEl.innerHTML = groups.map((item, index) => {
    ...
    return `
      <div class="student-donut-legend-item">
        ...
        <div class="student-donut-legend-text">
          ...
          <span>${escapeHtml(String(item.count))} ${unitLabel} • ${escapeHtml(percentage)}%</span>
        </div>
      </div>
    `;
  }).join("");
...
function renderPublicStudentsDonut() {
  ...
  const palette = [
    ...
    "#22c55e", "#a855f7", "#9ca3af", "#ec4899"
  ];

  renderDonutChart({
    chartEl: els.publicStudentsDonutChart,
    totalEl: els.publicStudentsTotal,
    legendEl: els.publicStudentsLegend,
    summaryEl: els.publicStudentsLegendSummary,
    groups,
    palette,
    unitLabel: "aluno(s)",
    emptyMessage: "Nenhum aluno cadastrado nas escolas até o momento.",
    summaryText: n => `${n} ${n === 1 ? "escola" : "escolas"} com alunos cadastrados`,
    emptySummaryText: "Nenhum aluno cadastrado."
  });
}
...
function renderPublicStaffDonut() {
  ...
  renderDonutChart({
    chartEl: els.publicStaffDonutChart,
    totalEl: els.publicStaffTotal,
    legendEl: els.publicStaffLegend,
    summaryEl: els.publicStaffLegendSummary,
    groups,
    palette,
    unitLabel: "registro(s)",
    emptyMessage: "Nenhum funcionário ou professor cadastrado nas escolas até o momento.",
    summaryText: n => `${n} ${n === 1 ? "escola" : "escolas"} com funcionários / professores cadastrados`,
    emptySummaryText: "Nenhum cadastro de funcionário ou professor."
  });
}
...
function getPresenceStatusClass(value = "") {
  ...
  if (value === "FJ") return "status-fj";
  if (value === "FN") return "status-fn";
  return "status-empty";
}
      ...
      <summary class="presence-person-summary">
        ...
        <div class="presence-person-summary-side">
          <div class="presence-person-summary-stats">
            ...
            <span class="presence-mini-stat"><strong>FJ</strong> ${summary.justified}</span>
            <span class="presence-mini-stat"><strong>FN</strong> ${summary.unjustified}</span>
...
function applyDefaultPresenceWorkdays() {
  ...
  els.presenceTableBody.querySelectorAll(".presence-day-select").forEach(select => {
    ...
    if (!select.value) {
      ...
      if (dayCard) {
        dayCard.classList.remove("status-empty", "status-fj", "status-fn");
...
function handlePresenceFieldChange(event) {
  ...
  const dayCard = select.closest(".presence-calendar-day");
  if (dayCard) {
    dayCard.classList.remove("status-empty", "status-p", "status-fj", "status-fn");
...
/** Alias de limitText com parâmetro padrão para uso em presença. */
