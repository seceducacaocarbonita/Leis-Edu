...
function mapElements() {
  [
    ...
    "institutionRecordsBody", "institutionForm", "institutionRecordId", "institutionName", "institutionType",
    "institutionRole", "institutionDepartment", "institutionPosition", "institutionStatus", "institutionCpf", "institutionAddress", "institutionPhone", "institutionEmail",
    "institutionVacation", "institutionLicense", "institutionLeaveStatus", "institutionLeaveFile", "institutionLeaveDocumentData", "institutionLeaveDocumentName", "institutionLeaveDocumentPreview",
    "clearInstitutionFormBtn", "institutionFormTitle", "institutionDetailEmpty", "institutionDetailCard", "printInstitutionRecordBtn",
...
function attachEvents() {
  ...
  els.institutionForm.addEventListener("submit", handleSaveInstitutionalRecord);
  els.printInstitutionRecordBtn.addEventListener("click", () => printInstitutionalRecord());
  els.institutionLeaveFile.addEventListener("change", handleInstitutionLeaveFileChange);
  els.clearFormBtn.addEventListener("click", clearDocumentForm);
  els.clearInstitutionFormBtn.addEventListener("click", clearInstitutionalForm);
...
function getInstitutionalSummary(rows = institutionalRecords) {
  ...
  const students = rows.filter(record => record.personType === "Aluno").length;
  const staff = rows.filter(record => record.personType === "Funcionário").length;
  const teachers = rows.filter(isProfessorRecord).length;
  const active = rows.filter(record => record.status === "Ativo").length;
  const away = rows.filter(record => record.status === "Afastado").length;
...
function syncPublicInstitutionalSummary() {
  settingsData.publicInstitutionalSummary = getInstitutionalSummary(
    institutionalRecords.map(normalizeInstitutionalRecord)
  );
}

function isProfessorRecord(record) {
  return record.personType === "Funcionário" && [record.role, record.position].some(value => normalizeText(value).includes("professor"));
}
...
function updateInstitutionLeaveDocumentPreview(data = "", name = "") {
  if (!els.institutionLeaveDocumentPreview) return;

  if (!data) {
    els.institutionLeaveDocumentPreview.textContent = "Nenhum documento anexado.";
    return;
  }

  const safeData = escapeHtml(data);
  const safeName = escapeHtml(name || "documento-afastamento");
  els.institutionLeaveDocumentPreview.innerHTML = `<a href="${safeData}" download="${safeName}" target="_blank" rel="noopener">${safeName}</a>`;
}

function handleInstitutionLeaveFileChange(event) {
  const file = event.target.files?.[0];
  if (!file) {
    els.institutionLeaveDocumentData.value = "";
    els.institutionLeaveDocumentName.value = "";
  ...
  reader.onload = () => {
    ...
    els.institutionLeaveDocumentData.value = result;
    els.institutionLeaveDocumentName.value = file.name;
...
async function handleSaveInstitutionalRecord(event) {
  event.preventDefault();

  const payload = {
    name: els.institutionName.value.trim(),
    personType: els.institutionType.value,
    role: els.institutionRole.value.trim(),
    department: els.institutionDepartment.value.trim(),
    position: els.institutionPosition.value.trim(),
    status: els.institutionStatus.value,
    vacation: els.institutionVacation.value,
    license: els.institutionLicense.value,
    leaveStatus: els.institutionLeaveStatus.value,
    leaveDocumentData: els.institutionLeaveDocumentData.value || "",
    leaveDocumentName: els.institutionLeaveDocumentName.value || "",
    cpf: formatCpf(els.institutionCpf.value),
    address: els.institutionAddress.value.trim(),
    phone: els.institutionPhone.value.trim(),
    email: els.institutionEmail.value.trim(),
    updatedAt: new Date().toISOString()
  };

  if (!payload.name || !payload.personType || !payload.role || !payload.department || !payload.position || !payload.status || !payload.cpf || !payload.address) {
    showToast("Preencha os campos obrigatórios da ficha institucional.");
    return;
  }

  if (payload.leaveStatus === "Sim" && !payload.leaveDocumentData) {
    showToast("Anexe o documento de afastamento em PNG ou PDF.");
    return;
  }

  if (payload.leaveStatus !== "Sim") {
  ...
  const duplicate = institutionalRecords.find(item => item.id !== els.institutionRecordId.value && normalizeCpf(item.cpf) === normalizedCpf);
  if (duplicate) {
    showToast("Já existe uma ficha cadastrada com esse CPF.");
    return;
  }

  const isEditing = Boolean(els.institutionRecordId.value);

  if (isEditing) {
    const index = institutionalRecords.findIndex(item => item.id === els.institutionRecordId.value);
    if (index >= 0) {
      ...
      showToast("Ficha institucional atualizada com sucesso.");
    ...
    showToast("Ficha institucional cadastrada com sucesso.");
...
function fillInstitutionalForm(record) {
  els.institutionRecordId.value = record.id || "";
  els.institutionName.value = record.name || "";
  els.institutionType.value = record.personType || "";
  els.institutionRole.value = record.role || "";
  els.institutionDepartment.value = record.department || "";
  els.institutionPosition.value = record.position || "";
  els.institutionStatus.value = record.status || "Ativo";
  els.institutionVacation.value = record.vacation || "";
  els.institutionLicense.value = record.license || "";
  els.institutionLeaveStatus.value = record.leaveStatus || "";
  els.institutionLeaveDocumentData.value = record.leaveDocumentData || "";
  els.institutionLeaveDocumentName.value = record.leaveDocumentName || "";
  updateInstitutionLeaveDocumentPreview(record.leaveDocumentData || "", record.leaveDocumentName || "");
  els.institutionCpf.value = record.cpf || "";
  els.institutionAddress.value = record.address || "";
  els.institutionPhone.value = record.phone || "";
  els.institutionEmail.value = record.email || "";
  els.institutionFormTitle.textContent = "Editar ficha institucional";
}

function clearInstitutionalForm() {
  els.institutionForm.reset();
  els.institutionRecordId.value = "";
  els.institutionStatus.value = "";
  els.institutionVacation.value = "";
  els.institutionLicense.value = "";
  els.institutionLeaveStatus.value = "";
  els.institutionLeaveDocumentData.value = "";
  els.institutionLeaveDocumentName.value = "";
  if (els.institutionLeaveFile) els.institutionLeaveFile.value = "";
  updateInstitutionLeaveDocumentPreview();
  els.institutionFormTitle.textContent = "Cadastrar ficha institucional";
}
