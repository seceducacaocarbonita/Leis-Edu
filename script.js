...
const ROLE_PERMISSIONS = {
  ...
  diretor: ["institutional", "staff-form", "presence", "student-form"]
};
...
function mapElements() {
  [
    ...
    "adminStatsGrid", "sphereBars", "recentDocumentsBody", "activityList", "alertSummaryGrid", "alertSchoolGrid", "alertList",
...
function renderDashboardTab() {
  ...
  safeRenderAdminSection(() => renderAdminAlerts(), () => {
    if (els.alertSummaryGrid) {
      els.alertSummaryGrid.innerHTML = [
        statCard("Total de alertas", 0),
        statCard("Críticos", 0),
        statCard("Atenção", 0),
        statCard("Informativos", 0)
      ].join("");
    }
    if (els.alertSchoolGrid) {
      els.alertSchoolGrid.innerHTML = '<div class="empty-card">Nenhum alerta por escola no momento.</div>';
    }
    if (els.alertList) {
      els.alertList.innerHTML = '<div class="empty-card">Nenhum alerta encontrado no momento.</div>';
    }
  }, "Painel de Alertas");
...
function renderAdminAlerts() {
  ...
  els.alertSummaryGrid.innerHTML = `
    ${statCard("Total de alertas", alerts.length)}
    ${statCard("Críticos", dangerCount)}
    ${statCard("Atenção", warningCount)}
    ${statCard("Informativos", infoCount)}
  `;
  ...
  if (els.alertSchoolGrid) {
    ...
    els.alertSchoolGrid.innerHTML = grouped.length ? grouped.map(item => `
      <article class="alert-school-card">
        <strong>${escapeHtml(item.label)}</strong>
        <span>${escapeHtml(String(item.count))} alerta(s)</span>
      </article>
    `).join("") : `
      <div class="empty-card">Nenhum alerta por escola no momento.</div>
    `;
  }

  if (!alerts.length) {
    els.alertList.innerHTML = `
      <div class="empty-card">Nenhum alerta encontrado no momento.</div>
    `;
    return;
  }

  els.alertList.innerHTML = alerts.slice(0, 18).map(alert => `
