...
async function saveInstitutionalData(deletedIds = []) {
  if (!adminLogged) return false;

  const idsToDelete = [...new Set((Array.isArray(deletedIds) ? deletedIds : []).filter(Boolean))];
  syncPublicInstitutionalSummary();

  try {
    const rows = institutionalRecords.map(toDbInstitutionalRecord);
    ...
    console.error("Erro ao salvar a ficha institucional no Supabase:", error);
    showToast("Não foi possível salvar a ficha no Supabase.");
    return false;
  }
  ...
  try {
    ...
    console.error("A ficha foi salva, mas houve falha ao atualizar os dados auxiliares:", error);
  }
...
async function handleInstitutionalTableClicks(event) {
  ...
  if (action === "delete-person") {
    ...
    const previousRecords = institutionalRecords.map(item => ({ ...item }));
    ...
    const saved = await saveInstitutionalData([record.id]);
    if (!saved) {
      institutionalRecords = previousRecords;
      syncPublicInstitutionalSummary();
      renderInstitutionalTable();
      renderInstitutionalDetail(currentInstitutionRecordId);
      renderAdminStats();
...
async function handleSaveInstitutionalRecord(event, mode = "staff") {
  ...
  const saved = await saveInstitutionalData();
  if (!saved) {
    institutionalRecords = previousRecords;
    syncPublicInstitutionalSummary();
    renderInstitutionalTable();
  ...
  showToast(
    mode === "student"
      ? (isEditing ? "Ficha de aluno atualizada com sucesso." : "Ficha de aluno cadastrada com sucesso.")
      : (isEditing ? "Ficha de funcionário / professor atualizada com sucesso." : "Ficha de funcionário / professor cadastrada com sucesso.")
  );
}
