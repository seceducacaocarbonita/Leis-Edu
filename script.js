...
const SECURITY_CONFIG = {
  ...
  maxLeaveDocumentBytes: 5 * 1024 * 1024,
  allowedCoverImageTypes: ["image/jpeg", "image/png", "image/webp"],
  allowedLeaveDocumentTypes: ["image/png", "application/pdf"]
};
...
function mapElements() {
  [
    ...
    "detailInstitutionName", "detailInstitutionType", "detailInstitutionRole", "detailInstitutionDepartment", "detailInstitutionPosition", "detailInstitutionFormation", "detailInstitutionConclusionDate",
...
function getAdminDataRequirementsForTab(tab = "dashboard") {
  ...
  switch (tab) {
    case "dashboard":
      requirements.ui = adminUserRole === "admin";
...
async function loadAdminDataset(datasetKey) {
  ...
  try {
    ...
    if (datasetKey === "institutional") {
      const { data, error } = await supabase.from(DB_TABLES.institutional).select("*").order("updated_at", { ascending: false });
      if (error) throw error;
    ...
    if (datasetKey === "attendance") {
      const { data, error } = await supabase.from(DB_TABLES.attendance).select("*").order("updated_at", { ascending: false });
      if (error) throw error;
      mergeAttendanceRowsIntoUi(data || []);
...
function getPersistableUiData() {
  return {
    theme: uiData.theme || "light",
    favorites: Array.isArray(uiData.favorites) ? uiData.favorites.slice(0, 50) : [],
    recentViews: Array.isArray(uiData.recentViews) ? uiData.recentViews.slice(0, 50) : []
  };
}
  ...
  try {
    if (adminUserRole === "admin") {
      await Promise.all([
        ...
        upsertSingleton(DB_TABLES.ui, UI_ROW_ID, getPersistableUiData()),
    ...
    if (adminUserRole === "diretor") {
      await syncCollectionTable(DB_TABLES.institutional, institutionalRecords.map(toDbInstitutionalRecord));
    }
  } catch (error) {
...
function openDetails(id) {
  ...
  const safePdfUrl = normalizeSafeUrl(doc.pdfUrl);
  if (safePdfUrl) {
    els.openPdfLink.href = safePdfUrl;
    els.openPdfLink.classList.remove("hidden");
    els.pdfFrame.src = safePdfUrl;
...
function getNoAccessProfile() {
  return { role: "none", school: "" };
}

function sanitizeRole(role = "") {
  return ROLE_PERMISSIONS[role] ? role : "none";
}

async function getUserAccessByEmail(email = "") {
  const normalizedEmail = normalizeEmail(email);
  if (!normalizedEmail) return getNoAccessProfile();

  try {
    const { data, error } = await supabase
      .from(DB_TABLES.users)
      .select("email, role, school, active")
      .eq("email", normalizedEmail)
      .maybeSingle();

    if (error) throw error;
    if (!data || data.active === false) return getNoAccessProfile();
    ...
    console.error("Erro ao consultar perfil de acesso no Supabase:", error);
    return getNoAccessProfile();
  }
}

function isKnownUserEmail(email = "") {
  // A autorização real fica no Supabase (tabela app_user_profiles + RLS).
  // Esta função é mantida apenas por compatibilidade com chamadas antigas.
...
async function enforceAuthorizedSession(email = "") {
  ...
  console.warn("Sessão bloqueada para e-mail sem perfil ativo no Supabase:", email);
  try {
...
function recordMatchesScopedSchool(record) {
  ...
  return getScopedSchoolValue(record?.department) === getScopedSchoolValue(adminUserSchool);
}
...
function lockDirectorSchoolFields() {
  ...
  [els.staffInstitutionDepartment, els.studentInstitutionDepartment, els.presenceSchoolFilter].forEach(field => {
    ...
    if (shouldLock && schoolValue) {
      const hasOption = Array.from(field.options || []).some(option => option.value === schoolValue);
...
function forceLocalLogoutState() {
  ...
  adminUserRole = "admin";
  adminUserSchool = "";
  userAccessProfile = getNoAccessProfile();
...
async function syncAuthSession() {
  try {
    ...
    const access = adminLogged ? userAccessProfile : getNoAccessProfile();
...
function toDbAttendanceRow(monthKey, person, entry) {
  const normalizedPerson = normalizeInstitutionalRecord(person);
  const normalizedEntry = normalizeDailyAttendanceEntry(entry);

  return {
    id: `${monthKey}:${normalizedPerson.id}`,
    person_id: normalizedPerson.id,
    month_key: monthKey,
    department: normalizedPerson.department || "Sem escola definida",
...
async function saveAttendanceRowsForMonth(monthKey, monthData, personCards = []) {
  if (!adminLogged) return;

  const payload = personCards
    .map(card => {
      const personId = card.dataset.personId || "";
      const person = institutionalRecords.find(item => item.id === personId);
      if (!person) return null;
      if (!recordMatchesScopedSchool(person)) return null;
      return toDbAttendanceRow(monthKey, person, monthData[personId]);
    })
    .filter(Boolean);
...
async function saveMonthlyPresence() {
  ...
  try {
    await saveAttendanceRowsForMonth(monthKey, nextMonthData, rows);
  } catch (error) {
    console.error("Erro ao salvar presença no Supabase:", error);
    showToast("Não foi possível salvar a presença no Supabase.");
    return;
  }
...
function renderInstitutionLeaveDocumentSlot(data = "", name = "") {
  ...
  if (!isSafeLeaveDocumentDataUrl(data)) {
    els.detailInstitutionLeaveDocumentWrap.textContent = "Documento bloqueado por validação de segurança.";
    return;
  }

  els.detailInstitutionLeaveDocumentWrap.innerHTML = `<a class="btn btn-light btn-full" href="${escapeHtml(data)}" download="${escapeHtml(name || "documento-afastamento")}" target="_blank" rel="noopener noreferrer">Abrir documento anexado</a>`;
}
...
function updateInstitutionLeaveDocumentPreview(data = "", name = "") {
  ...
  if (!isSafeLeaveDocumentDataUrl(data)) {
    els.staffInstitutionLeaveDocumentPreview.textContent = "Documento bloqueado por validação de segurança.";
    return;
  }

  const safeData = escapeHtml(data);
  const safeName = escapeHtml(name || "documento-afastamento");
  els.staffInstitutionLeaveDocumentPreview.innerHTML = `<a href="${safeData}" download="${safeName}" target="_blank" rel="noopener noreferrer">${safeName}</a>`;
}
...
function handleInstitutionLeaveFileChange(event, mode = "staff") {
  ...
  const allowedTypes = SECURITY_CONFIG.allowedLeaveDocumentTypes;
  const lowerName = file.name.toLowerCase();
  const isPngByName = lowerName.endsWith(".png");
  const isPdfByName = lowerName.endsWith(".pdf");

  if (file.size > SECURITY_CONFIG.maxLeaveDocumentBytes) {
    showToast("O documento deve ter no máximo 5 MB.");
    event.target.value = "";
    return;
  }

  if (!allowedTypes.includes(file.type) || (!isPngByName && !isPdfByName)) {
    showToast("Anexe somente arquivo PNG ou PDF válido.");
    event.target.value = "";
    return;
  }
  ...
  reader.onload = () => {
    ...
    if (!isSafeLeaveDocumentDataUrl(result)) {
      showToast("O arquivo anexado não passou na validação de segurança.");
      event.target.value = "";
      return;
    }
...
async function handleInstitutionalTableClicks(event) {
  ...
  if (!userCanAccessTab("institutional") && !userCanAccessTab("staff-form") && !userCanAccessTab("student-form")) {
    showToast("Seu perfil não tem permissão para acessar fichas institucionais.");
    return;
  }
...
async function handleSaveDocument(event) {
  ...
  const payload = {
    ...
    pdfUrl: normalizeSafeUrl(els.pdfUrl.value.trim()),
    coverImageData: isSafeImageDataUrl(els.coverImageData.value) ? els.coverImageData.value : "",
  ...
  if (els.pdfUrl.value.trim() && !payload.pdfUrl) {
    showToast("Informe um link de PDF seguro começando com https:// ou http://.");
    return;
  }
...
function handleCoverUpload(event) {
  ...
  const allowedTypes = SECURITY_CONFIG.allowedCoverImageTypes;
  const lowerName = file.name.toLowerCase();
  const hasAllowedExtension = [".jpg", ".jpeg", ".png", ".webp"].some(ext => lowerName.endsWith(ext));

  if (file.size > SECURITY_CONFIG.maxCoverImageBytes) {
    showToast("A imagem de capa deve ter no máximo 2 MB.");
    event.target.value = "";
    return;
  }

  if (!allowedTypes.includes(file.type) || !hasAllowedExtension) {
    showToast("Selecione apenas imagem JPG, PNG ou WEBP válida.");
    event.target.value = "";
    return;
  }
  ...
  reader.onload = () => {
    ...
    if (!isSafeImageDataUrl(result)) {
      showToast("A imagem não passou na validação de segurança.");
      event.target.value = "";
      return;
    }
...
async function persistAuditLog(log) {
  ...
  try {
    const { error } = await supabase
      ...
      .upsert(toDbAuditLog(log), { onConflict: "id" });
...
function coverStyle(dataUrl) {
  return isSafeImageDataUrl(dataUrl) ? `background-image:url("${dataUrl}")` : "";
}
...
function isSafeImageDataUrl(value = "") {
  const raw = String(value || "").trim();
  return /^data:image\/(png|jpeg|jpg|webp);base64,[a-z0-9+/=\s]+$/i.test(raw);
}

function isSafeLeaveDocumentDataUrl(value = "") {
  const raw = String(value || "").trim();
  return /^data:(application\/pdf|image\/png);base64,[a-z0-9+/=\s]+$/i.test(raw);
}
