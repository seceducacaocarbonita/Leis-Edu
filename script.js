...
function mapElements() {
  [
    ...
    "presenceMonthPicker", "presenceSchoolFilter", "fillPresenceWorkdaysBtn", "savePresenceBtn", "presenceMonthInfo", "presenceSummaryGrid", "presenceTableHead", "presenceTableBody",
...
function renderInstitutionalTable() {
  ...
  els.institutionRecordsBody.innerHTML = rows.length ? rows.map(record => {
    ...
    return `
    <tr class="institution-row ${supportTeacher ? "student-has-support" : ""}">
      <td class="institution-cell-name">
        <div class="institution-person-cell">
          <strong class="institution-person-name">${escapeHtml(record.name)}</strong>
          ${getStudentSupportBadge(record)}
        </div>
      </td>
      <td class="institution-cell-type">${getInstitutionTypeBadge(record)}</td>
      <td class="institution-cell-school"><span>${escapeHtml(record.department || "-")}</span></td>
      <td class="institution-cell-position"><span>${escapeHtml(record.position || "-")}</span></td>
      <td class="institution-cell-status">${getInstitutionStatusBadge(record.status)}</td>
      <td class="institution-cell-cpf">${escapeHtml(formatCpf(record.cpf) || "-")}</td>
      <td class="institution-cell-actions">
      ...
      <summary class="presence-person-summary">
        ...
        <div class="presence-person-summary-side">
          <div class="presence-person-summary-stats">
            <span class="presence-mini-stat"><strong>P</strong> ${summary.present}</span>
            <span class="presence-mini-stat"><strong>A</strong> ${summary.justified}</span>
            <span class="presence-mini-stat"><strong>AJ</strong> ${summary.unjustified}</span>
            <span class="presence-mini-stat">${getAttendanceRateBadge(summary.rate)}</span>
          </div>
...
function applyDefaultPresenceWorkdays() {
  if (!els.presenceTableBody) return;

  const monthKey = getPresenceMonthKey();

  els.presenceTableBody.querySelectorAll(".presence-day-select").forEach(select => {
    ...
    if (!select.value) {
      ...
      if (dayCard) {
        dayCard.classList.remove("status-empty", "status-a", "status-aj");
        dayCard.classList.add("status-p");
      }
    }
  });

  showToast("Dias úteis em branco preenchidos como Presente.");
}
...
function renderInstitutionalDetail(recordId) {
  ...
  setText(els.detailInstitutionCpf, formatCpf(record.cpf) || "-");
  ...
  setText(els.detailInstitutionPhone, formatPhone(record.phone) || "-");
...
function fillInstitutionalForm(record) {
  ...
  if (isStudent) {
    ...
    if (els.studentInstitutionCpf) els.studentInstitutionCpf.value = formatCpf(normalized.cpf || "");
    els.studentInstitutionAddress.value = normalized.address || "";
    els.studentInstitutionPhone.value = formatPhone(normalized.phone || "");
  ...
  els.staffInstitutionCpf.value = formatCpf(normalized.cpf || "");
  els.staffInstitutionAddress.value = normalized.address || "";
  els.staffInstitutionPhone.value = formatPhone(normalized.phone || "");
...
function formatPhone(value) {
  ...
  if (number.length <= firstBlockSize) {
    return `(${ddd}) ${number}`;
  }
...
function setMaskedInputValue(input, formatter) {
  if (!input || typeof formatter !== "function") return;

  const selectionStart = input.selectionStart ?? input.value.length;
  const digitsBeforeCursor = onlyDigits(input.value.slice(0, selectionStart)).length;
...
function bindMaskedInput(input, formatter) {
  ...
  input.addEventListener("input", () => setMaskedInputValue(input, formatter));
...
function bindInstitutionalInputMasks() {
  bindMaskedInput(els.staffInstitutionPhone, formatPhone);
  bindMaskedInput(els.studentInstitutionPhone, formatPhone);
  bindMaskedInput(els.staffInstitutionCpf, formatCpf);

  [els.staffInstitutionPhone, els.studentInstitutionPhone].forEach(input => {
    if (!input) return;
  ...
  if (els.staffInstitutionCpf) {
    els.staffInstitutionCpf.setAttribute("inputmode", "numeric");
    els.staffInstitutionCpf.setAttribute("maxlength", "14");
    els.staffInstitutionCpf.setAttribute("autocomplete", "off");
    els.staffInstitutionCpf.setAttribute("placeholder", "123.456.789-10");
  }
}
