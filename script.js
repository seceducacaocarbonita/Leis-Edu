...
function buildInstitutionalPayload(mode) {
  ...
  if (mode === "student") {
    return {
      ...
      personType: "Aluno",
      role: els.studentInstitutionRole.value.trim(),
...
function validateInstitutionalPayload(payload, mode) {
  if (!payload.name || !payload.role || !payload.department || !payload.position || !payload.status || !payload.cpf || !payload.address) {
    showToast(mode === "student" ? "Preencha os campos obrigatórios da ficha do aluno." : "Preencha os campos obrigatórios da ficha de funcionário / professor.");
    return false;
  }
...
async function handleInstitutionalTableClicks(event) {
  ...
  if (action === "edit-person") {
    ...
    renderInstitutionalDetail(record.id);
    switchAdminTab(normalizeInstitutionalRecord(record).personType === "Aluno" ? "student-form" : "institutional");
    return;
  }
