...
function renderAdminAlerts() {
  if (!els.alertSummaryGrid || !els.alertList) return;
  ...
  if (els.miniAlertTotal) {
    els.miniAlertTotal.textContent = String(alerts.length);
    els.miniAlertTotal.setAttribute("aria-label", `${alerts.length} alerta(s)`);
  }

  if (els.miniAlertScope) {
    els.miniAlertScope.textContent = getAlertScopeLabel();
  }

  els.alertSummaryGrid.innerHTML = renderMiniAlertSummary(dangerCount, warningCount, infoCount);

  if (els.alertSchoolGrid) {
    if (isSchoolScopedDirector()) {
      els.alertSchoolGrid.innerHTML = `
      ...
      els.alertSchoolGrid.innerHTML = grouped.length ? grouped.map(item => `
        <div class="mini-alert-school-chip">
