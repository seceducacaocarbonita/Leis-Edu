...
function renderPresencePanel() {
  ...
  if (els.presenceMonthInfo) {
    els.presenceMonthInfo.textContent = `Calendário diário referente a ${label}. Sábados e domingos aparecem destacados.`;
  }
    ...
    const dayCells = Array.from({ length: totalDays }, (_, index) => {
      ...
      return `
        <td class="presence-day-cell ${weekend ? "is-weekend" : ""}">
          <select class="presence-day-select" data-day="${dayKey}" aria-label="Dia ${Number(dayKey)} de ${escapeHtml(person.name)}">
