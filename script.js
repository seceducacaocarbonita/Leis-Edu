...
function mapElements() {
  [
    ...
    "presenceMonthPicker", "presenceSearchInput", "fillPresenceWorkdaysBtn", "savePresenceBtn", "presenceMonthInfo", "presenceSummaryGrid", "presenceTableHead", "presenceTableBody",
...
function getPresenceStatusMeta(status = "") {
  const map = {
    "": { label: "—", className: "is-empty", title: "Sem lançamento" },
    P: { label: "P", className: "is-p", title: "Presente" },
    FJ: { label: "FJ", className: "is-fj", title: "Falta justificada" },
    FN: { label: "FN", className: "is-fn", title: "Falta não justificada" }
  };

  return map[status] || map[""];
}

function getNextPresenceStatus(currentStatus = "") {
  const sequence = ["", "P", "FJ", "FN"];
  const currentIndex = sequence.indexOf(currentStatus);
  return sequence[(currentIndex + 1) % sequence.length];
}

function buildPresenceDayButton(dayKey, personName, value = "", weekend = false) {
  const meta = getPresenceStatusMeta(value);
  return `
    <button
      class="presence-day-btn ${meta.className} ${weekend ? "is-weekend" : ""}"
      type="button"
      data-day="${dayKey}"
      data-status="${value}"
      aria-label="Dia ${Number(dayKey)} de ${escapeHtml(personName)}: ${meta.title}"
      title="${meta.title}"
    >${meta.label}</button>
  `;
}
...
function handlePresenceTableClicks(event) {
  const button = event.target.closest(".presence-day-btn");
  if (!button) return;

  const nextStatus = getNextPresenceStatus(button.dataset.status || "");
  const meta = getPresenceStatusMeta(nextStatus);

  button.dataset.status = nextStatus;
  button.textContent = meta.label;
  button.title = meta.title;
  button.setAttribute("aria-label", (button.getAttribute("aria-label") || "").replace(/:.*$/, `: ${meta.title}`));
...
function renderPresencePanel() {
  ...
  const searchTerm = normalizeText(els.presenceSearchInput?.value || "");
  ...
  if (els.presenceMonthInfo) {
    els.presenceMonthInfo.textContent = `Calendário diário referente a ${label}. Clique nas células para alternar o status. Sábados e domingos ficam destacados.`;
  }
  ...
  els.presenceTableHead.innerHTML = `
    <tr>
      <th rowspan="2" class="presence-sticky-col presence-sticky-col-1">Servidor</th>
      <th rowspan="2" class="presence-sticky-col presence-sticky-col-2">Escola / lotação</th>
      <th colspan="${totalDays}">Dias do mês</th>
      <th rowspan="2">P</th>
      <th rowspan="2">FJ</th>
      <th rowspan="2">FN</th>
  ...
  if (!allPeople.length) {
    ...
    els.presenceTableBody.innerHTML = `
      <tr>
        <td colspan="${totalDays + 7}"><div class="empty-card">Cadastre primeiro funcionários ou professores para lançar a presença diária.</div></td>
      </tr>
    `;
    return;
  }

  const allPeopleSummaries = allPeople.map(person => {
  ...
  const visiblePeopleSummaries = allPeopleSummaries.filter(item => {
    if (!searchTerm) return true;
    return normalizeText(`${item.person.name} ${item.person.department || ""} ${item.person.position || ""} ${item.person.personType || ""}`).includes(searchTerm);
  });

  const launchedPeople = allPeopleSummaries.filter(item => item.summary.launched > 0);
  const averageRate = launchedPeople.length
    ? launchedPeople.reduce((sum, item) => sum + (item.summary.rate || 0), 0) / launchedPeople.length
    : 0;
  const totalPresent = allPeopleSummaries.reduce((sum, item) => sum + item.summary.present, 0);
  const totalJustified = allPeopleSummaries.reduce((sum, item) => sum + item.summary.justified, 0);
  const totalUnjustified = allPeopleSummaries.reduce((sum, item) => sum + item.summary.unjustified, 0);

  els.presenceSummaryGrid.innerHTML = `
    ${statCard("Funcionários / Professores", allPeople.length)}
    ${statCard("Exibidos na tela", visiblePeopleSummaries.length)}
  ...
  if (!visiblePeopleSummaries.length) {
    els.presenceTableBody.innerHTML = `
      <tr>
        <td colspan="${totalDays + 7}"><div class="empty-card">Nenhum servidor encontrado para o filtro informado.</div></td>
      </tr>
    `;
    return;
  }

  els.presenceTableBody.innerHTML = visiblePeopleSummaries.map(({ person, entry, summary }) => {
    const dayCells = Array.from({ length: totalDays }, (_, index) => {
      ...
      return `
        <td class="presence-day-cell ${weekend ? "is-weekend" : ""}">
          ${buildPresenceDayButton(dayKey, person.name, currentValue, weekend)}
        </td>
      `;
    }).join("");
    ...
    return `
      <tr data-person-id="${person.id}">
        <td class="presence-sticky-col presence-sticky-col-1 presence-person-cell">
          <div class="presence-person-name">${escapeHtml(person.name)}</div>
          <div class="presence-person-meta">
            ${getInstitutionTypeBadge(person)}
            <span>${escapeHtml(person.position || "Sem cargo")}</span>
          </div>
        </td>
        <td class="presence-sticky-col presence-sticky-col-2 presence-department-cell">${escapeHtml(person.department || "Não informado")}</td>
        ${dayCells}
...
function applyDefaultPresenceWorkdays() {
  ...
  els.presenceTableBody.querySelectorAll(".presence-day-btn").forEach(button => {
    ...
    if (!button.dataset.status) {
      const meta = getPresenceStatusMeta("P");
...
async function saveMonthlyPresence() {
  ...
  for (const row of rows) {
    ...
    const nextDays = { ...previousEntry.days };
