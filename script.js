      ...
      <summary class="presence-person-summary">
        ...
        <div class="presence-person-summary-side">
          <div class="presence-person-summary-stats">
            <span class="presence-mini-stat status-p"><strong>P</strong> ${summary.present}</span>
            <span class="presence-mini-stat status-a"><strong>A</strong> ${summary.unjustified}</span>
            <span class="presence-mini-stat status-aj"><strong>AJ</strong> ${summary.justified}</span>
            <span class="presence-mini-stat status-rate">${getAttendanceRateBadge(summary.rate)}</span>
          </div>
...
function applyDefaultPresenceWorkdays() {
  ...
  if (!updatedDays) {
    showToast("Nenhum dia útil disponível para preencher neste mês.");
    return;
  }

  showToast("Todos os dias úteis, de segunda a sexta, foram preenchidos como Presente. Clique em Salvar para gravar.");
}

function clearMonthlyPresenceSelection() {
  if (!els.presenceTableBody) return;

  const selects = [...els.presenceTableBody.querySelectorAll(".presence-day-select")];
  const hasValues = selects.some(select => !!select.value);

  if (!hasValues) {
    showToast("Não há presença lançada para limpar na tela atual.");
    return;
  }

  const confirmed = window.confirm("Deseja limpar todos os lançamentos de presença exibidos neste mês? Depois clique em Salvar para gravar a alteração.");
  if (!confirmed) return;
  ...
  showToast("Presença limpa na tela atual. Clique em Salvar para gravar.");
}
