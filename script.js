...
function mapElements() {
  [
    ...
    "institutionalStatsGrid", "institutionSearchInput", "institutionTypeFilter", "institutionRoleFilter",
    "institutionRecordsBody", "institutionForm", "institutionRecordId", "institutionName", "institutionType",
    "institutionRole", "institutionCpf", "institutionAddress", "institutionPhone", "institutionEmail",
    "clearInstitutionFormBtn", "institutionFormTitle", "institutionDetailEmpty", "institutionDetailCard",
    "detailInstitutionName", "detailInstitutionType", "detailInstitutionRole", "detailInstitutionCpf",
...
function attachEvents() {
  ...
  els.institutionRoleFilter.addEventListener("change", renderInstitutionalTable);
...
async function loadData() {
  ...
  try {
    ...
    settingsData = settingsResponse.data?.data || createDefaultSettings();
...
function getInstitutionalSummary(rows = institutionalRecords) {
  ...
  return { total, students, staff, teachers };
}

function isProfessorRecord(record) {
  return record.personType === "Funcionário" && normalizeText(record.role).includes("professor");
}
...
function getFilteredInstitutionalRecords() {
  ...
  const roleFilter = els.institutionRoleFilter?.value || "";
  ...
  return [...institutionalRecords]
    .filter(record => {
      ...
      const matchesType = !type || record.personType === type;
      const isTeacher = isProfessorRecord(record);
      const matchesRole = !roleFilter || (roleFilter === "professor" ? isTeacher : !isTeacher);

      return matchesTerm && matchesType && matchesRole;
    })
    .sort((a, b) => a.name.localeCompare(b.name, "pt-BR"));
}
...
function renderInstitutionalTable() {
  ...
  els.institutionRecordsBody.innerHTML = rows.length ? rows.map(record => `
    <tr>
      ...
      <td>${record.personType === "Funcionário" ? `<span class="badge badge-yellow">Funcionário</span>` : `<span class="badge">Aluno</span>`}</td>
...
function renderInstitutionalDetail(recordId) {
  const record = institutionalRecords.find(item => item.id === recordId);
  ...
  setText(els.detailInstitutionType, record.personType || "-");
...
async function handleSaveInstitutionalRecord(event) {
  ...
  if (!payload.name || !payload.personType || !payload.role || !payload.cpf || !payload.address) {
...
function exportJsonBackup() {
  ...
  const a = document.createElement("a");
  a.href = url;
  a.download = `consulta-leis-v7-backup-${new Date().toISOString().slice(0, 10)}.json`;
  a.click();
...
function importJsonBackup(event) {
  ...
  reader.onload = () => {
    try {
      ...
      institutionalRecords = Array.isArray(data.institutionalRecords) ? data.institutionalRecords : [];
      protocolCounter = documentsData.length + 1;
      saveData();
      clearDocumentForm();
      clearCategoryForm();
