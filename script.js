...
function attachEvents() {
  ...
  els.openAdminBtn.addEventListener("click", () => {
    if (adminLogged) {
      renderAdminState();
      switchAdminTab(currentTab || "dashboard");
    }
...
function clearAdminVisualState() {
  if (els.adminStatsGrid) els.adminStatsGrid.innerHTML = "";
  if (els.alertSummaryGrid) els.alertSummaryGrid.innerHTML = "";
  if (els.alertSchoolGrid) els.alertSchoolGrid.innerHTML = "";
  if (els.alertList) els.alertList.innerHTML = "";
  if (els.sphereBars) els.sphereBars.innerHTML = "";
  if (els.recentDocumentsBody) els.recentDocumentsBody.innerHTML = "";
  if (els.activityList) els.activityList.innerHTML = "";
  if (els.adminDocumentsBody) els.adminDocumentsBody.innerHTML = "";
  if (els.categoriesBody) els.categoriesBody.innerHTML = "";
  if (els.institutionalStatsGrid) els.institutionalStatsGrid.innerHTML = "";
  if (els.institutionRecordsBody) els.institutionRecordsBody.innerHTML = "";
  if (els.presenceSummaryGrid) els.presenceSummaryGrid.innerHTML = "";
  if (els.presenceTableBody) els.presenceTableBody.innerHTML = "";
  if (els.reportSummaryGrid) els.reportSummaryGrid.innerHTML = "";
  if (els.reportTableBody) els.reportTableBody.innerHTML = "";
  if (els.auditTableBody) els.auditTableBody.innerHTML = "";
}

function renderAdminForCurrentSession() {
  renderAdminState();

  if (!adminLogged) {
    clearAdminVisualState();
    return;
  }
...
function getUserAccessByEmail(email = "") {
  ...
  if (normalizedEmail.includes("diretor")) {
    return {
      role: "diretor",
      school: "Escola Municipal de Abadia"
    };
  }
...
async function syncAuthSession() {
  try {
    ...
    const access = adminLogged ? getUserAccessByEmail(adminUserEmail) : { role: "admin", school: "" };
    adminUserRole = access.role;
    adminUserSchool = access.school || "";
...
function switchAdminTab(tab) {
  ...
  const adminContentScroll = document.querySelector(".admin-content");
  if (adminContentScroll) {
    adminContentScroll.scrollTo({ top: 0, behavior: "smooth" });
  }
