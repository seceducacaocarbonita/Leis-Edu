...
async function loadAdminDataset(datasetKey) {
  ...
  try {
    ...
    if (datasetKey === "institutional") {
      ...
      if (error) throw error;
      institutionalRecords = (data || []).map(fromDbInstitutionalRecord);
...
async function deleteInstitutionalRecordFromDb(record) {
  ...
  // Diretor continua limitado à própria escola, mas a exclusão no Supabase deve ser feita pelo ID.
  // Antes usávamos também filtro exato por `department`; quando o nome da escola tinha pequena variação
  // de grafia/acentuação, o Supabase não apagava nenhuma linha e o cadastro voltava ao recarregar.
  ...
  if (!Array.isArray(data) || data.length === 0) {
    throw new Error("Nenhum registro foi excluído no Supabase.");
  }
}
...
function recordMatchesScopedSchool(record) {
  ...
  return getScopedSchoolVariants(adminUserSchool)
    .map(normalizeSchoolScope)
    .includes(recordSchool);
}
...
async function handleInstitutionalTableClicks(event) {
  ...
  if (action === "delete-person") {
    ...
    try {
      await deleteInstitutionalRecordFromDb(record);
    } catch (error) {
      console.error("Erro ao excluir ficha institucional:", error);
      showToast("Não foi possível excluir a ficha institucional.");
      return;
    }
