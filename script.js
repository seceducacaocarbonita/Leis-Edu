...
const USER_ACCESS_BY_EMAIL = {
  ...
  "escoladaestiva2@gmail.com": {
    role: "diretor",
    school: "Escola Municipal Nuclêo da Estiva"
  },
  "emeiprefeitojoselemos.carbonita@gmail.com": {
    role: "diretor",
    school: "Escola Municipal Prefeito José Lemos"
  },
  "karinamartinsleite1985@gmail.com": {
    role: "diretor",
    school: "Escola Municipal Antônia Pereira Rocha"
  }
...
function observeRevealElements() {
  const revealItems = document.querySelectorAll(".panel, .stat-card, .doc-card, .featured-card, .category-card, .alert-card, .alert-school-card, .presence-person-card");
...
(function () {
  ...
  function closeAllCustomSelects(exceptInstance = null) {
    customSelectState.instances.forEach(instance => {
  ...
  function getSelectedOption(select) {
    return Array.from(select.options).find(option => option.selected) || select.options[0] || null;
  }
    ...
    const instance = {
      ...
      rebuild() {
        ...
        options.forEach((option, index) => {
          ...
          instance.optionButtons.push(optionBtn);
        });

        if (instance.highlightedIndex < 0 && instance.optionButtons.length) {
          instance.highlightedIndex = Math.max(0, select.selectedIndex);
        }
        instance.syncHighlight();
      },
      syncHighlight() {
        instance.optionButtons.forEach((btn, idx) => {
          btn.classList.toggle("is-active", idx === instance.highlightedIndex && !btn.classList.contains("is-selected"));
        });
      },
      open() {
        ...
        instance.highlightedIndex = Math.max(0, select.selectedIndex);
        instance.syncHighlight();
        customSelectState.active = instance;
      },
      close() {
        wrapper.classList.remove("open");
        trigger.setAttribute("aria-expanded", "false");
        customSelectState.active = customSelectState.active === instance ? null : customSelectState.active;
      },
      toggle() {
      ...
      move(step) {
        const enabledButtons = instance.optionButtons
          .map((btn, idx) => ({ btn, idx }))
          .filter(item => !item.btn.disabled);

        if (!enabledButtons.length) return;

        let currentEnabledIndex = enabledButtons.findIndex(item => item.idx === instance.highlightedIndex);
        if (currentEnabledIndex === -1) currentEnabledIndex = 0;
        currentEnabledIndex = (currentEnabledIndex + step + enabledButtons.length) % enabledButtons.length;
        instance.highlightedIndex = enabledButtons[currentEnabledIndex].idx;
        instance.syncHighlight();
        enabledButtons[currentEnabledIndex].btn.scrollIntoView({ block: "nearest" });
      },
      commitHighlighted() {
        const btn = instance.optionButtons[instance.highlightedIndex];
        if (btn && !btn.disabled) {
          btn.click();
        }
      }
  ...
  function initCustomSelects(scope = document) {
    const selects = scope.querySelectorAll('select:not([multiple]):not([data-native-select="true"])');
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    const bodyObserver = new MutationObserver(() => {
      initCustomSelects();
      customSelectState.instances.forEach(instance => instance.rebuild());
    });
...
(function () {
  ...
  function buildCustomSelect(select) {
    ...
    const instance = {
      ...
      refresh() {
        ...
        value.textContent = options[selectedIndex] ? options[selectedIndex].textContent.trim() : "";
      ...
      paintHighlight() {
        this.buttons.forEach((btn, idx) => {
          ...
          btn.classList.toggle("is-active", idx === this.highlightedIndex && !isSelected);
        });
      },
      open() {
        closeAll(this);
      ...
      toggle() {
        if (wrapper.classList.contains("open")) this.close();
        else this.open();
      },
      move(step) {
        const enabled = this.buttons
          .map((btn, idx) => ({ btn, idx }))
          .filter(item => !item.btn.disabled);

        if (!enabled.length) return;

        let pos = enabled.findIndex(item => item.idx == this.highlightedIndex);
        if (pos < 0) pos = 0;
        pos = (pos + step + enabled.length) % enabled.length;
        this.highlightedIndex = enabled[pos].idx;
        this.paintHighlight();
        enabled[pos].btn.scrollIntoView({ block: "nearest" });
      },
      chooseHighlighted() {
        const btn = this.buttons[this.highlightedIndex];
    ...
    trigger.addEventListener("keydown", (event) => {
      if (event.key === "ArrowDown") {
        event.preventDefault();
        if (!wrapper.classList.contains("open")) instance.open();
        else instance.move(1);
      } else if (event.key === "ArrowUp") {
        event.preventDefault();
        if (!wrapper.classList.contains("open")) instance.open();
        else instance.move(-1);
        ...
        event.preventDefault();
        if (!wrapper.classList.contains("open")) instance.open();
        else instance.chooseHighlighted();
      } else if (event.key === "Escape") {
        instance.close();
      }
    });
    ...
    const observer = new MutationObserver(() => instance.refresh());
    observer.observe(select, {
      childList: true,
      subtree: true,
      attributes: true,
  ...
  function initCustomSelects(root = document) {
    root.querySelectorAll('select:not([multiple])').forEach(buildCustomSelect);
  }
  ...
  if (previousRenderAll) {
    window.renderAll = function (...args) {
      ...
      setTimeout(() => initCustomSelects(document), 0);
      return result;
    };
  }

  const previousFillSelect = typeof window.fillSelect === "function" ? window.fillSelect : null;
  if (previousFillSelect) {
    window.fillSelect = function (...args) {
      const result = previousFillSelect.apply(this, args);
      const select = args[0];
      if (select) {
