...
function mapElements() {
  [
    ...
    "presenceMonthPicker", "presenceSearchInput", "fillPresenceWorkdaysBtn", "savePresenceBtn", "presenceMonthInfo", "presenceSummaryGrid", "presenceTableBody",
...
function attachEvents() {
  ...
  els.presenceSearchInput?.addEventListener("input", renderPresencePanel);
  ...
  els.presenceTableBody?.addEventListener("click", handlePresenceTableClick);
  els.presenceTableBody?.addEventListener("input", handlePresenceTableInput);
...
function ensureMonthlyAttendanceStore() {
  if (!uiData || typeof uiData !== "object") {
    uiData = createDefaultUiState();
  }

  if (!uiData.monthlyAttendance || typeof uiData.monthlyAttendance !== "object" || Array.isArray(uiData.monthlyAttendance)) {
    uiData.monthlyAttendance = {};
  }

  return uiData.monthlyAttendance;
}
...
function normalizeAttendanceEntry(entry = {}) {
  ...
  return {
    expectedDays: parseNumber(entry.expectedDays, 22),
    presentDays: parseNumber(entry.presentDays, ""),
    justifiedAbsences: parseNumber(entry.justifiedAbsences, 0),
    unjustifiedAbsences: parseNumber(entry.unjustifiedAbsences, 0),
...
function getMonthlyAttendanceForMonth(monthKey, autoCreate = true) {
  const store = ensureMonthlyAttendanceStore();
  ...
  if (autoCreate) {
    getPresenceEligibleRecords().forEach(record => {
      current[record.id] = normalizeAttendanceEntry(current[record.id]);
    });
...
function calculateAttendanceRate(entry) {
  const expected = Number(entry?.expectedDays || 0);
  const present = entry?.presentDays === "" ? null : Number(entry?.presentDays || 0);

  if (!expected || present === null || !Number.isFinite(present)) return null;
  return Math.max(0, Math.min(100, (present / expected) * 100));
}

function getPresenceWorkdaysForMonth(monthKey = getPresenceMonthKey()) {
  if (!/^\d{4}-\d{2}$/.test(monthKey || "")) return 22;
  const [year, month] = monthKey.split("-").map(Number);
...
function getPresenceRowNumericValue(row, selector, fallback = 0, allowBlank = false) {
...
function refreshPresenceRowRate(row) {
  if (!row) return;
  const entry = normalizeAttendanceEntry({
    expectedDays: getPresenceRowNumericValue(row, ".js-presence-expected", 0),
    presentDays: getPresenceRowNumericValue(row, ".js-presence-present", "", true),
    justifiedAbsences: getPresenceRowNumericValue(row, ".js-presence-justified", 0),
    unjustifiedAbsences: getPresenceRowNumericValue(row, ".js-presence-unjustified", 0)
  });
  const rateCell = row.querySelector("[data-presence-rate]");
  if (rateCell) {
    rateCell.innerHTML = getAttendanceRateBadge(calculateAttendanceRate(entry));
  }
}

function refreshPresenceSummaryCardsFromInputs() {
  ...
  const entries = rows.map(row => normalizeAttendanceEntry({
    expectedDays: getPresenceRowNumericValue(row, ".js-presence-expected", 0),
    presentDays: getPresenceRowNumericValue(row, ".js-presence-present", "", true),
    justifiedAbsences: getPresenceRowNumericValue(row, ".js-presence-justified", 0),
    unjustifiedAbsences: getPresenceRowNumericValue(row, ".js-presence-unjustified", 0)
  ...
  const launchedEntries = entries.filter(entry => entry.presentDays !== "");
  const averageRate = launchedEntries.length
    ? launchedEntries.reduce((sum, entry) => sum + (calculateAttendanceRate(entry) || 0), 0) / launchedEntries.length
    : 0;
  const totalAbsences = entries.reduce((sum, entry) => {
    return sum + Number(entry.justifiedAbsences || 0) + Number(entry.unjustifiedAbsences || 0);
  }, 0);

  els.presenceSummaryGrid.innerHTML = `
    ${statCard("Funcionários / Professores", rows.length)}
    ${statCard("Lançamentos concluídos", launchedEntries.length)}
    ${statCard("Média de frequência", launchedEntries.length ? formatPercent(averageRate) : "0%")}
    ${statCard("Faltas registradas", totalAbsences)}
  `;
}

function applyPresenceShortcutToRow(row, mode) {
  if (!row) return;
  const workdays = getPresenceWorkdaysForMonth();
  const expectedInput = row.querySelector(".js-presence-expected");
  const presentInput = row.querySelector(".js-presence-present");
  const justifiedInput = row.querySelector(".js-presence-justified");
  const unjustifiedInput = row.querySelector(".js-presence-unjustified");

  if (!expectedInput || !presentInput || !justifiedInput || !unjustifiedInput) return;

  if (mode === "present") {
    expectedInput.value = String(workdays);
    presentInput.value = String(workdays);
    justifiedInput.value = "0";
    unjustifiedInput.value = "0";
  } else if (mode === "justified") {
    expectedInput.value = String(workdays);
    presentInput.value = "0";
    justifiedInput.value = String(workdays);
    unjustifiedInput.value = "0";
  } else if (mode === "unjustified") {
    expectedInput.value = String(workdays);
    presentInput.value = "0";
    justifiedInput.value = "0";
    unjustifiedInput.value = String(workdays);
  } else if (mode === "clear") {
    expectedInput.value = String(workdays);
    presentInput.value = "";
    justifiedInput.value = "0";
  ...
  refreshPresenceRowRate(row);
  refreshPresenceSummaryCardsFromInputs();
}

function handlePresenceTableClick(event) {
  const shortcutButton = event.target.closest(".js-presence-shortcut");
  if (!shortcutButton) return;
  const row = shortcutButton.closest("tr[data-person-id]");
  applyPresenceShortcutToRow(row, shortcutButton.dataset.shortcut || "");
}

function handlePresenceTableInput(event) {
  const input = event.target.closest(".js-presence-expected, .js-presence-present, .js-presence-justified, .js-presence-unjustified");
  if (!input) return;
  const row = input.closest("tr[data-person-id]");
  refreshPresenceRowRate(row);
  refreshPresenceSummaryCardsFromInputs();
}
...
function renderPresencePanel() {
  ...
  const monthData = getMonthlyAttendanceForMonth(monthKey, true);
  const searchTerm = (els.presenceSearchInput?.value || "").trim().toLocaleLowerCase("pt-BR");
  const people = searchTerm
    ? allPeople.filter(person => person.name.toLocaleLowerCase("pt-BR").includes(searchTerm))
    : allPeople;

  if (!allPeople.length) {
    if (els.presenceMonthInfo) {
      els.presenceMonthInfo.textContent = `Lançamentos de presença referentes a ${label}.`;
    }
    ...
    els.presenceSummaryGrid.innerHTML = `
      ...
      ${statCard("Faltas registradas", 0)}
    `;

    els.presenceTableBody.innerHTML = `
      <tr>
        <td colspan="10"><div class="empty-card">Cadastre primeiro funcionários ou professores para lançar a presença mensal.</div></td>
      </tr>
    `;
    return;
  }
  ...
  if (els.presenceMonthInfo) {
    els.presenceMonthInfo.textContent = searchTerm
      ? `Lançamentos de presença referentes a ${label}. Exibindo ${people.length} servidor(es) encontrados para “${els.presenceSearchInput.value.trim()}”.`
      : `Lançamentos de presença referentes a ${label}.`;
  }
  ...
  if (!people.length) {
    ...
    els.presenceTableBody.innerHTML = `
      <tr>
        <td colspan="10"><div class="empty-card">Nenhum servidor encontrado com esse nome.</div></td>
      </tr>
    `;
    return;
  }

  const launchedEntries = people
    .map(person => ({ person, entry: normalizeAttendanceEntry(monthData[person.id]) }))
    .filter(item => item.entry.presentDays !== "");

  const averageRate = launchedEntries.length
    ? launchedEntries.reduce((sum, item) => sum + (calculateAttendanceRate(item.entry) || 0), 0) / launchedEntries.length
    : 0;

  const totalAbsences = launchedEntries.reduce((sum, item) => {
    return sum + Number(item.entry.justifiedAbsences || 0) + Number(item.entry.unjustifiedAbsences || 0);
  }, 0);
  ...
  els.presenceTableBody.innerHTML = people.map(person => {
    const entry = normalizeAttendanceEntry(monthData[person.id]);
    const rate = calculateAttendanceRate(entry);
      ...
      <tr data-person-id="${person.id}">
        <td>
          <div class="presence-person-main">
            ...
            <div class="presence-row-shortcuts">
              <button class="presence-shortcut-btn js-presence-shortcut" type="button" data-shortcut="present">Úteis = P</button>
              <button class="presence-shortcut-btn js-presence-shortcut" type="button" data-shortcut="justified">Úteis = FJ</button>
              <button class="presence-shortcut-btn js-presence-shortcut" type="button" data-shortcut="unjustified">Úteis = FN</button>
              <button class="presence-shortcut-btn ghost js-presence-shortcut" type="button" data-shortcut="clear">Limpar</button>
            </div>
          </div>
        </td>
        ...
        <td><input class="presence-cell-input js-presence-expected" type="number" min="0" value="${entry.expectedDays === "" ? "" : entry.expectedDays}" /></td>
        <td><input class="presence-cell-input js-presence-present" type="number" min="0" value="${entry.presentDays === "" ? "" : entry.presentDays}" /></td>
        <td><input class="presence-cell-input js-presence-justified" type="number" min="0" value="${entry.justifiedAbsences}" /></td>
        <td><input class="presence-cell-input js-presence-unjustified" type="number" min="0" value="${entry.unjustifiedAbsences}" /></td>
        <td data-presence-rate>${getAttendanceRateBadge(rate)}</td>
...
function applyDefaultPresenceWorkdays() {
  ...
  const workdays = getPresenceWorkdaysForMonth();
  els.presenceTableBody.querySelectorAll(".js-presence-expected").forEach(input => {
    input.value = String(workdays);
  });

  refreshPresenceSummaryCardsFromInputs();
  showToast(`Dias previstos preenchidos com ${workdays} dias úteis para o mês selecionado.`);
}
...
async function saveMonthlyPresence() {
  ...
  const existingMonthData = getMonthlyAttendanceForMonth(monthKey, true);
  const nextMonthData = { ...existingMonthData };
  const rows = [...els.presenceTableBody.querySelectorAll("tr[data-person-id]")];

  for (const row of rows) {
    const personId = row.dataset.personId || "";
    const expectedRaw = row.querySelector(".js-presence-expected")?.value ?? "";
    const presentRaw = row.querySelector(".js-presence-present")?.value ?? "";
    const justifiedRaw = row.querySelector(".js-presence-justified")?.value ?? "0";
    const unjustifiedRaw = row.querySelector(".js-presence-unjustified")?.value ?? "0";
    const observations = (row.querySelector(".js-presence-observations")?.value || "").trim();

    const expectedDays = expectedRaw === "" ? 0 : Number(expectedRaw);
    const presentDays = presentRaw === "" ? "" : Number(presentRaw);
    const justifiedAbsences = justifiedRaw === "" ? 0 : Number(justifiedRaw);
    const unjustifiedAbsences = unjustifiedRaw === "" ? 0 : Number(unjustifiedRaw);

    const invalid = [expectedDays, justifiedAbsences, unjustifiedAbsences].some(value => !Number.isFinite(value) || value < 0)
      || (presentDays !== "" && (!Number.isFinite(presentDays) || presentDays < 0));

    if (invalid) {
      showToast("Informe apenas números válidos na aba Presença.");
      return;
    }

    if (presentDays !== "" && expectedDays > 0 && presentDays + justifiedAbsences + unjustifiedAbsences > expectedDays) {
      showToast("A soma de presenças e faltas não pode ultrapassar os dias previstos.");
      return;
    }

    nextMonthData[personId] = normalizeAttendanceEntry({
      expectedDays,
      presentDays,
      justifiedAbsences,
      unjustifiedAbsences,
  ...
  ensureMonthlyAttendanceStore()[monthKey] = nextMonthData;
  addAudit("Atualização", "Presença", monthLabel, "Presença mensal de funcionários e professores atualizada");
  await saveData();
  renderPresencePanel();
  renderActivityList();
  renderAuditTable();
  showToast(`Presença de ${monthLabel} salva com sucesso.`);
}
