...
let currentPresenceRenderToken = 0;
...
function mapElements() {
  [
    ...
    "presenceMonthPicker", "presenceSchoolFilter", "fillPresenceWorkdaysBtn", "savePresenceBtn", "presenceMonthInfo", "presenceSummaryGrid", "presenceTableHead", "presenceTableBody", "presenceListMeta", "expandPresenceCardsBtn", "collapsePresenceCardsBtn",
...
function attachEvents() {
  ...
  els.expandPresenceCardsBtn?.addEventListener("click", expandAllPresenceCards);
  els.collapsePresenceCardsBtn?.addEventListener("click", collapseAllPresenceCards);
...
function updatePresenceDayCardState(select) {
...
function updatePresenceListMeta(totalPeople = 0, monthKey = "", schoolLabel = "") {
  if (!els.presenceListMeta) return;
  const label = formatPresenceMonthLabel(monthKey || getPresenceMonthKey());
  const schoolText = schoolLabel ? ` • ${schoolLabel}` : "";
  els.presenceListMeta.textContent = `${totalPeople} ficha(s) exibida(s) • ${label}${schoolText}`;
}

function renderPresenceCardsChunked(peopleSummaries, monthKey) {
  if (!els.presenceTableBody) return;
  const renderToken = ++currentPresenceRenderToken;
  const batchSize = 10;
  let cursor = 0;
  ...
  const renderBatch = () => {
    if (renderToken !== currentPresenceRenderToken) return;

    const slice = peopleSummaries.slice(cursor, cursor + batchSize);
    if (!slice.length) return;

    const html = slice.map(({ person, entry, summary }) => `
      <details
    ...
    els.presenceTableBody.insertAdjacentHTML("beforeend", html);
    cursor += batchSize;

    if (cursor < peopleSummaries.length) {
...
function expandAllPresenceCards() {
  if (!els.presenceTableBody) return;
  els.presenceTableBody.querySelectorAll(".presence-person-card").forEach(card => {
    card.open = true;
  });
}

function collapseAllPresenceCards() {
  if (!els.presenceTableBody) return;
  els.presenceTableBody.querySelectorAll(".presence-person-card").forEach(card => {
    card.open = false;
  });
...
function queuePresenceSummaryRefresh() {
  ...
  requestAnimationFrame(() => {
    ...
    const people = getPresenceFilteredRecords(getPresenceEligibleRecords());

    const peopleSummaries = people.map(person => {
      const row = els.presenceTableBody?.querySelector(`.presence-person-card[data-person-id="${CSS.escape(person.id)}"]`);
      const entry = row
        ? normalizeDailyAttendanceEntry({
            days: Object.fromEntries(
              [...row.querySelectorAll(".presence-day-select")]
                .map(select => [select.dataset.day || "", select.value || ""])
                .filter(([dayKey, value]) => /^\d{2}$/.test(dayKey) && ["P", "A", "AJ"].includes(value))
            ),
            observations: row.dataset.observations || "",
            updatedAt: new Date().toISOString()
          })
        : normalizeDailyAttendanceEntry(monthData[person.id]);
    ...
    updatePresenceListMeta(people.length, monthKey, getPresenceSchoolFilterValue());
  });
}
...
function renderPresencePanel() {
  ...
  updatePresenceListMeta(people.length, monthKey, schoolFilterLabel);
  renderPresenceCardsChunked(peopleSummaries, monthKey);
}
...
function applyDefaultPresenceWorkdays() {
  ...
  els.presenceTableBody.querySelectorAll(".presence-day-select").forEach(select => {
    ...
    if (!select.value) {
      select.value = "P";
      updatePresenceDayCardState(select);
      presenceDirty = true;
    }
  });

  queuePresenceSummaryRefresh();
...
async function saveMonthlyPresence() {
  if (!adminLogged) return;
  if (!els.presenceTableBody || !els.savePresenceBtn) return;
  ...
  try {
    els.savePresenceBtn.disabled = true;
    els.savePresenceBtn.textContent = "Salvando...";
    ...
    els.savePresenceBtn.disabled = false;
    els.savePresenceBtn.textContent = "Salvar";
  }
}
