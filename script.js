...
function renderPresencePanel() {
  ...
  if (els.presenceMonthInfo) {
    els.presenceMonthInfo.textContent = `Calendário diário referente a ${label}. Nome, escola e cargo permanecem fixos durante a rolagem horizontal. Sábados e domingos aparecem destacados.`;
  }
  ...
  els.presenceTableHead.innerHTML = `
    <tr>
      <th rowspan="2" class="presence-col-name presence-sticky presence-sticky-1">Nome</th>
      <th rowspan="2" class="presence-col-school presence-sticky presence-sticky-2">Escola</th>
      <th rowspan="2" class="presence-col-position presence-sticky presence-sticky-3">Cargo</th>
  ...
  els.presenceTableBody.innerHTML = peopleSummaries.map(({ person, entry, summary }) => {
    ...
    return `
      <tr data-person-id="${person.id}">
        <td class="presence-col-name presence-sticky presence-sticky-1"><strong>${escapeHtml(person.name)}</strong></td>
        <td class="presence-col-school presence-sticky presence-sticky-2">${escapeHtml(person.department || "-")}</td>
        <td class="presence-col-position presence-sticky presence-sticky-3">${escapeHtml(person.position || "-")}</td>
        <td class="presence-col-type">${getInstitutionTypeBadge(person)}</td>
        ${dayCells}
