...
async function loadData() {
  try {
    ...
    const storedSettings = JSON.parse(localStorage.getItem(STORAGE_KEYS.settings) || "null");
    const storedUi = JSON.parse(localStorage.getItem(STORAGE_KEYS.ui) || "null");
    const storedAudit = JSON.parse(localStorage.getItem(STORAGE_KEYS.audit) || "null");

    if (Array.isArray(storedDocuments) && Array.isArray(storedCategories) && storedSettings && storedUi && Array.isArray(storedAudit)) {
      documentsData = storedDocuments;
    ...
    console.warn("Não foi possível carregar os dados salvos. A base de exemplo será restaurada.", error);
  }
