...
function renderAll() {
  ...
  renderPublicDocuments();
  renderAdminState();
  // A aba administrativa é renderizada por switchAdminTab().
  // Evita renderização duplicada que recriava os cards depois da animação,
  // deixando estatísticas e alertas invisíveis no login do admin.
...
function revealAdminModalContent() {
  if (!els.adminModal || els.adminModal.classList.contains("hidden")) return;

  els.adminModal
    .querySelectorAll(".panel, .stat-card, .alert-card, .alert-school-card, .presence-person-card")
