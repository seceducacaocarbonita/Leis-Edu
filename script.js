...
let currentPresenceView = "monthly";
let currentPresencePersonId = "";
let presenceDraftMonthKey = "";
let presenceDraftMonthData = {};
...
function mapElements() {
  [
    ...
    "presenceMonthlyViewBtn", "presenceIndividualViewBtn", "presenceMonthlyPanel", "presenceIndividualPanel",
    "presenceMonthlySearchInput", "presenceServerPicker", "presenceIndividualEmpty", "presenceIndividualCard",
    "presenceIndividualMeta", "presenceIndividualStats", "presenceIndividualObservations", "presenceIndividualDays",
...
function attachEvents() {
  ...
  els.presenceMonthPicker.addEventListener("change", () => {
    presenceDraftMonthKey = "";
    renderPresencePanel();
  });
  ...
  els.presenceMonthlyViewBtn.addEventListener("click", () => setPresenceView("monthly"));
  els.presenceIndividualViewBtn.addEventListener("click", () => setPresenceView("individual"));
  els.presenceMonthlySearchInput.addEventListener("input", renderPresenceMonthlyTable);
  els.presenceServerPicker.addEventListener("change", () => {
    currentPresencePersonId = els.presenceServerPicker.value || "";
    renderPresenceIndividualView();
  });
  els.presenceTableBody.addEventListener("change", handlePresenceMonthlyTableChange);
  els.presenceTableBody.addEventListener("click", handlePresenceMonthlyTableClick);
  els.presenceIndividualDays.addEventListener("click", handlePresenceIndividualDaysClick);
  els.presenceIndividualObservations.addEventListener("change", handlePresenceIndividualObservationsChange);
...
function getPresenceStatusClass(value = "") {
  if (value === "P") return "is-present";
  if (value === "FJ") return "is-justified";
  if (value === "FN") return "is-unjustified";
  return "is-empty";
}
...
function clonePresenceMonthData(source = {}, people = []) {
  const next = {};
  people.forEach(person => {
    next[person.id] = normalizeDailyAttendanceEntry(source?.[person.id]);
  });
  return next;
}

function ensurePresenceDraftMonthData() {
  const monthKey = getPresenceMonthKey();
  const people = getPresenceEligibleRecords();
  const source = presenceDraftMonthKey === monthKey
    ? presenceDraftMonthData
    : getDailyAttendanceForMonth(monthKey, true);

  presenceDraftMonthKey = monthKey;
  presenceDraftMonthData = clonePresenceMonthData(source, people);

  if (!currentPresencePersonId || !people.some(person => person.id === currentPresencePersonId)) {
    currentPresencePersonId = people[0]?.id || "";
  }

  return presenceDraftMonthData;
}

function getPresenceDraftEntry(personId) {
  const monthData = ensurePresenceDraftMonthData();
  if (!monthData[personId]) {
    monthData[personId] = normalizeDailyAttendanceEntry();
  }
  return monthData[personId];
}

function updatePresenceDraftDay(personId, dayKey, value) {
  if (!personId || !/^\d{2}$/.test(dayKey || "")) return;
  const entry = getPresenceDraftEntry(personId);

  if (["P", "FJ", "FN"].includes(value)) {
    entry.days[dayKey] = value;
  } else {
...
function updatePresenceDraftObservations(personId, observations = "") {
  if (!personId) return;
  const entry = getPresenceDraftEntry(personId);
  entry.observations = observations.trim();
  entry.updatedAt = new Date().toISOString();
}

function setPresenceView(view = "monthly") {
  currentPresenceView = view === "individual" ? "individual" : "monthly";

  els.presenceMonthlyViewBtn?.classList.toggle("active", currentPresenceView === "monthly");
  els.presenceIndividualViewBtn?.classList.toggle("active", currentPresenceView === "individual");
  els.presenceMonthlyPanel?.classList.toggle("hidden", currentPresenceView !== "monthly");
  els.presenceIndividualPanel?.classList.toggle("hidden", currentPresenceView !== "individual");
}

function renderPresenceSummary(people, monthKey, monthData) {
  const totalDays = getDaysInMonth(monthKey);
...
function renderPresenceMonthlyTable() {
  if (!els.presenceTableBody || !els.presenceTableHead) return;

  const monthKey = getPresenceMonthKey();
  const totalDays = getDaysInMonth(monthKey);
  const people = getPresenceEligibleRecords();
  const monthData = ensurePresenceDraftMonthData();
  const query = normalizeText(els.presenceMonthlySearchInput?.value || "");
  const filteredPeople = query
    ? people.filter(person => [person.name, person.personType, person.department, person.position].some(value => normalizeText(value || "").includes(query)))
    : people;
  ...
  els.presenceTableHead.innerHTML = `
    <tr>
      <th rowspan="2">Servidor</th>
      <th rowspan="2">Tipo</th>
      <th rowspan="2">Escola</th>
      <th rowspan="2">Cargo</th>
      <th colspan="${totalDays}">Calendário diário</th>
  ...
  if (!filteredPeople.length) {
    els.presenceTableBody.innerHTML = `
      <tr>
        <td colspan="${totalDays + 10}"><div class="empty-card">Nenhum servidor encontrado com o filtro informado.</div></td>
      </tr>
    `;
    return;
  }

  els.presenceTableBody.innerHTML = filteredPeople.map(person => {
    const entry = normalizeDailyAttendanceEntry(monthData[person.id]);
    const summary = calculateDailyAttendanceSummary(entry, monthKey);
    const dayCells = Array.from({ length: totalDays }, (_, index) => {
      ...
      return `
        <td class="presence-day-cell ${weekend ? "is-weekend" : ""}">
          <select class="presence-day-select ${getPresenceStatusClass(currentValue)}" data-day="${dayKey}" aria-label="Dia ${Number(dayKey)} de ${escapeHtml(person.name)}">
    ...
    return `
      <tr data-person-id="${person.id}">
        <td>
          <button class="presence-open-person-btn" type="button">
            <strong>${escapeHtml(person.name)}</strong>
            <small>${escapeHtml(person.position || person.role || person.personType || "Servidor")}</small>
          </button>
        </td>
        <td>${getInstitutionTypeBadge(person)}</td>
        <td>${escapeHtml(person.department || "-")}</td>
        <td>${escapeHtml(person.position || "-")}</td>
        ${dayCells}
        <td><strong>${summary.present}</strong></td>
        <td>${summary.justified}</td>
        <td>${summary.unjustified}</td>
        <td>${getAttendanceRateBadge(summary.rate)}</td>
        <td><input class="presence-text-input js-presence-observations" type="text" value="${escapeHtml(entry.observations)}" placeholder="Observações" /></td>
      </tr>
    `;
  }).join("");
}

function renderPresenceServerPicker(people) {
  if (!els.presenceServerPicker) return;

  const options = people.map(person => {
    const selected = person.id === currentPresencePersonId ? "selected" : "";
    const school = person.department ? ` — ${escapeHtml(person.department)}` : "";
    return `<option value="${escapeHtml(person.id)}" ${selected}>${escapeHtml(person.name)}${school}</option>`;
  }).join("");

  els.presenceServerPicker.innerHTML = options || '<option value="">Nenhum servidor disponível</option>';
  if (currentPresencePersonId) {
    els.presenceServerPicker.value = currentPresencePersonId;
  }
}

function getPlainAttendanceRate(rate) {
  return rate === null ? "Não lançado" : formatPercent(rate);
}

function renderPresenceIndividualView() {
  if (!els.presenceIndividualEmpty || !els.presenceIndividualCard || !els.presenceIndividualMeta || !els.presenceIndividualDays || !els.presenceIndividualObservations || !els.presenceIndividualStats) return;

  const monthKey = getPresenceMonthKey();
  const people = getPresenceEligibleRecords();
  const monthData = ensurePresenceDraftMonthData();

  renderPresenceServerPicker(people);

  if (!people.length || !currentPresencePersonId) {
    els.presenceIndividualEmpty.classList.remove("hidden");
    els.presenceIndividualCard.classList.add("hidden");
    return;
  }

  const person = people.find(item => item.id === currentPresencePersonId);
  if (!person) {
    els.presenceIndividualEmpty.classList.remove("hidden");
    els.presenceIndividualCard.classList.add("hidden");
    return;
  ...
  els.presenceIndividualEmpty.classList.add("hidden");
  els.presenceIndividualCard.classList.remove("hidden");
  els.presenceIndividualMeta.innerHTML = `
    <div class="presence-individual-identity">
      <h4>${escapeHtml(person.name)}</h4>
      <p>${escapeHtml(person.personType || "Servidor")} • ${escapeHtml(person.position || "Cargo não informado")}</p>
    </div>
    <div class="presence-individual-lotation">
      <span>Escola / lotação</span>
      <strong>${escapeHtml(person.department || "Não informada")}</strong>
    </div>
  `;

  els.presenceIndividualStats.innerHTML = `
    ${statCard("Dias úteis", summary.workdays)}
    ${statCard("Presenças", summary.present)}
    ${statCard("FJ", summary.justified)}
    ${statCard("FN", summary.unjustified)}
    ${statCard("Frequência", getPlainAttendanceRate(summary.rate))}
  `;

  els.presenceIndividualObservations.value = entry.observations || "";

  els.presenceIndividualDays.innerHTML = Array.from({ length: totalDays }, (_, index) => {
    const dayKey = String(index + 1).padStart(2, "0");
    const status = entry.days[dayKey] || "";
    const weekend = isWeekendPresenceDay(monthKey, dayKey);
    return `
      <article class="presence-day-card ${weekend ? "is-weekend" : ""}">
        <div class="presence-day-card-top">
          <strong>${dayKey}</strong>
          <span>${getPresenceDayWeekLabel(monthKey, dayKey)}</span>
        </div>
        <button class="presence-cycle-btn ${getPresenceStatusClass(status)}" type="button" data-person-id="${person.id}" data-day="${dayKey}" data-current-status="${status}">
          ${getPresenceStatusLabel(status)}
        </button>
      </article>
    `;
  }).join("");
...
function renderPresencePanel() {
  ...
  if (els.presenceMonthInfo) {
    els.presenceMonthInfo.textContent = `Controle de presença referente a ${label}. Use a visão mensal para lançar em lote e a visão individual para revisar um servidor específico.`;
  }

  renderPresenceSummary(people, monthKey, monthData);
  renderPresenceMonthlyTable();
  renderPresenceIndividualView();
  setPresenceView(currentPresenceView);
}

function handlePresenceMonthlyTableChange(event) {
  const row = event.target.closest("tr[data-person-id]");
  if (!row) return;

  const personId = row.dataset.personId || "";

  if (event.target.matches(".presence-day-select")) {
    const dayKey = event.target.dataset.day || "";
    updatePresenceDraftDay(personId, dayKey, event.target.value || "");
    event.target.className = `presence-day-select ${getPresenceStatusClass(event.target.value || "")}`;
    renderPresenceSummary(getPresenceEligibleRecords(), getPresenceMonthKey(), ensurePresenceDraftMonthData());
    if (currentPresencePersonId === personId) {
      renderPresenceIndividualView();
    }
  }

  if (event.target.matches(".js-presence-observations")) {
    updatePresenceDraftObservations(personId, event.target.value || "");
    if (currentPresencePersonId === personId) {
      renderPresenceIndividualView();
    }
  }
...
function handlePresenceMonthlyTableClick(event) {
  const button = event.target.closest(".presence-open-person-btn");
  if (!button) return;

  const row = button.closest("tr[data-person-id]");
  if (!row) return;

  currentPresencePersonId = row.dataset.personId || "";
  renderPresenceIndividualView();
  setPresenceView("individual");
}

function getNextPresenceStatus(current = "") {
  const order = ["", "P", "FJ", "FN"];
  const currentIndex = order.indexOf(current);
  return order[(currentIndex + 1) % order.length];
}

function handlePresenceIndividualDaysClick(event) {
  const button = event.target.closest(".presence-cycle-btn");
  if (!button) return;

  const personId = button.dataset.personId || currentPresencePersonId;
  const dayKey = button.dataset.day || "";
  const nextStatus = getNextPresenceStatus(button.dataset.currentStatus || "");

  updatePresenceDraftDay(personId, dayKey, nextStatus);
  renderPresenceSummary(getPresenceEligibleRecords(), getPresenceMonthKey(), ensurePresenceDraftMonthData());
  renderPresenceMonthlyTable();
  renderPresenceIndividualView();
}

function handlePresenceIndividualObservationsChange(event) {
  if (!currentPresencePersonId) return;
  updatePresenceDraftObservations(currentPresencePersonId, event.target.value || "");
  renderPresenceMonthlyTable();
}
...
function applyDefaultPresenceWorkdays() {
  ...
  people.forEach(person => {
    const entry = getPresenceDraftEntry(person.id);
    for (let day = 1; day <= totalDays; day += 1) {
      const dayKey = String(day).padStart(2, "0");
      if (isWeekendPresenceDay(monthKey, dayKey)) continue;
      if (!entry.days[dayKey]) {
        entry.days[dayKey] = "P";
      }
    }
    entry.updatedAt = new Date().toISOString();
...
async function saveMonthlyPresence() {
  ...
  store[monthKey] = clonePresenceMonthData(monthData, people);
