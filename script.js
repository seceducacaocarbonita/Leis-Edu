...
let presenceDragState = { active: false, value: "", touchedRows: new Set() };
...
function mapElements() {
  [
    ...
    "presenceMonthPicker", "fillPresenceWorkdaysBtn", "savePresenceBtn", "presenceMonthInfo", "presenceSummaryGrid", "presenceTableHead", "presenceTableBody",
...
function attachEvents() {
  ...
  els.presenceTableBody.addEventListener("mousedown", handlePresenceTableMouseDown);
  els.presenceTableBody.addEventListener("mouseover", handlePresenceTableMouseOver);
  document.addEventListener("mouseup", stopPresenceDrag);
  window.addEventListener("blur", stopPresenceDrag);
...
function normalizeDailyAttendanceEntry(entry = {}) {
  const allowedValues = new Set(["P", "FJ", "FN"]);
...
function calculateDailyAttendanceSummary(entry, monthKey) {
  ...
  for (let day = 1; day <= totalDays; day += 1) {
    ...
    if (status === "FJ") justified += 1;
    if (status === "FN") unjustified += 1;
  }
...
const PRESENCE_STATUS_ORDER = ["", "P", "FJ", "FN"];

function getPresenceStatusLabel(value = "") {
  if (value === "P") return "P";
  if (value === "FJ") return "FJ";
  if (value === "FN") return "FN";
  return "—";
}

function getPresenceStatusTitle(value = "") {
  if (value === "P") return "Presente";
  if (value === "FJ") return "Falta justificada";
  if (value === "FN") return "Falta não justificada";
  return "Sem lançamento";
}

function getPresenceCellButtonMarkup(value = "", personName = "", dayKey = "") {
  const safeValue = ["P", "FJ", "FN"].includes(value) ? value : "";
  const label = getPresenceStatusLabel(safeValue);
  const title = `${personName} — dia ${Number(dayKey)}: ${getPresenceStatusTitle(safeValue)}. Clique para alterar.`;

  return `
    <button
      type="button"
      class="presence-day-btn is-${safeValue || "empty"}"
      data-day="${dayKey}"
      data-status="${safeValue}"
...
function getNextPresenceStatus(currentValue = "") {
  const index = PRESENCE_STATUS_ORDER.indexOf(currentValue);
  const nextIndex = index === -1 ? 1 : (index + 1) % PRESENCE_STATUS_ORDER.length;
  return PRESENCE_STATUS_ORDER[nextIndex];
}

function updatePresenceDayButton(button, nextValue = "", personName = "") {
  if (!button) return;
  const safeValue = ["P", "FJ", "FN"].includes(nextValue) ? nextValue : "";
  const dayKey = button.dataset.day || "";
  button.dataset.status = safeValue;
  button.classList.remove("is-empty", "is-P", "is-FJ", "is-FN");
  button.classList.add(`is-${safeValue || "empty"}`);
  button.textContent = getPresenceStatusLabel(safeValue);
  const title = `${personName} — dia ${Number(dayKey)}: ${getPresenceStatusTitle(safeValue)}. Clique ou arraste para alterar.`;
  button.setAttribute("aria-label", title);
  button.title = title;
}

function applyPresenceButtonValue(button, nextValue = "") {
  if (!button) return;
  const row = button.closest("tr[data-person-id]");
  const personName = row?.querySelector(".presence-name-cell strong")?.textContent?.trim() || "Servidor";
  updatePresenceDayButton(button, nextValue, personName);
  if (row) {
    presenceDragState.touchedRows.add(row);
    refreshPresenceRowSummary(row);
  }
  refreshPresenceSummaryCards();
}

function startPresenceDrag(button) {
  const currentValue = button?.dataset.status || "";
  presenceDragState.active = true;
  presenceDragState.value = getNextPresenceStatus(currentValue);
  presenceDragState.touchedRows = new Set();
  document.body.classList.add("presence-dragging");
}

function stopPresenceDrag() {
  if (!presenceDragState.active) return;
  presenceDragState.active = false;
  presenceDragState.value = "";
  presenceDragState.touchedRows = new Set();
  document.body.classList.remove("presence-dragging");
}

function handlePresenceTableMouseDown(event) {
  const dayButton = event.target.closest(".presence-day-btn");
  if (!dayButton || event.button !== 0) return;
  event.preventDefault();
  dayButton.dataset.skipClick = "1";
  startPresenceDrag(dayButton);
  applyPresenceButtonValue(dayButton, presenceDragState.value);
}

function handlePresenceTableMouseOver(event) {
  if (!presenceDragState.active) return;
  const dayButton = event.target.closest(".presence-day-btn");
  if (!dayButton) return;
  if (dayButton.dataset.status === presenceDragState.value) return;
  applyPresenceButtonValue(dayButton, presenceDragState.value);
}

function renderPresencePanel() {
  if (!els.presenceMonthPicker || !els.presenceTableBody || !els.presenceSummaryGrid || !els.presenceTableHead) return;
  ...
  const totalDays = getDaysInMonth(monthKey);
  const people = getPresenceEligibleRecords();
  ...
  if (els.presenceMonthInfo) {
    els.presenceMonthInfo.textContent = `Calendário diário referente a ${label}. Clique ou arraste pelas células para preencher vários dias com o mesmo status. Sábados e domingos aparecem destacados.`;
  }
  ...
  const dayHeaders = Array.from({ length: totalDays }, (_, index) => {
    ...
    return `
      <th class="presence-day-head ${weekend ? "is-weekend" : ""}">
        <span>${dayKey}</span>
        <small>${getPresenceDayWeekLabel(monthKey, dayKey)}</small>
      </th>
    `;
  }).join("");

  els.presenceTableHead.innerHTML = `
    <tr>
      <th rowspan="2" class="presence-col-name presence-sticky-col presence-sticky-col-1">Nome</th>
      <th rowspan="2" class="presence-col-school presence-sticky-col presence-sticky-col-2">Escola</th>
      <th rowspan="2" class="presence-col-type">Tipo</th>
      <th rowspan="2" class="presence-col-role">Cargo</th>
      <th colspan="${totalDays}" class="presence-calendar-group-head">Calendário diário</th>
      <th rowspan="2" class="presence-col-summary">Pres.</th>
      <th rowspan="2" class="presence-col-summary">FJ</th>
      <th rowspan="2" class="presence-col-summary">FN</th>
      <th rowspan="2" class="presence-col-frequency">Frequência</th>
      <th rowspan="2" class="presence-col-notes" title="Observações">Obs.</th>
    </tr>
    <tr>${dayHeaders}</tr>
  `;
  ...
  if (!people.length) {
    ...
    els.presenceTableBody.innerHTML = `
      <tr>
        <td colspan="${totalDays + 10}"><div class="empty-card">Cadastre primeiro funcionários ou professores para lançar a presença diária.</div></td>
      </tr>
    `;
    return;
  }
  ...
  els.presenceTableBody.innerHTML = peopleSummaries.map(({ person, entry, summary }) => {
    const dayCells = Array.from({ length: totalDays }, (_, index) => {
      ...
      return `
        <td class="presence-day-cell ${weekend ? "is-weekend" : ""}">
          ${getPresenceCellButtonMarkup(currentValue, person.name, dayKey)}
        </td>
      `;
    }).join("");

    return `
      <tr data-person-id="${person.id}" data-observations="${escapeHtml(entry.observations)}">
        <td class="presence-name-cell presence-sticky-col presence-sticky-col-1"><strong>${escapeHtml(person.name)}</strong></td>
        <td class="presence-school-cell presence-sticky-col presence-sticky-col-2">${escapeHtml(person.department || "-")}</td>
        <td class="presence-type-cell">${getInstitutionTypeBadge(person)}</td>
        <td class="presence-role-cell"><span class="presence-role-text" title="${escapeHtml(person.position || "-")}">${escapeHtml(person.position || "-")}</span></td>
        ${dayCells}
        <td data-summary="present"><strong>${summary.present}</strong></td>
        <td data-summary="justified">${summary.justified}</td>
        <td data-summary="unjustified">${summary.unjustified}</td>
        <td data-summary="rate">${getAttendanceRateBadge(summary.rate)}</td>
        <td class="presence-note-cell">
          <button
...
function applyDefaultPresenceWorkdays() {
  ...
  els.presenceTableBody.querySelectorAll(".presence-day-btn").forEach(button => {
    const dayKey = button.dataset.day || "";
    if (!dayKey || isWeekendPresenceDay(monthKey, dayKey)) return;
    if (!button.dataset.status) {
      const row = button.closest("tr[data-person-id]");
      const personName = row?.querySelector(".presence-name-cell strong")?.textContent?.trim() || "Servidor";
      updatePresenceDayButton(button, "P", personName);
    }
  });

  els.presenceTableBody.querySelectorAll("tr[data-person-id]").forEach(refreshPresenceRowSummary);
  refreshPresenceSummaryCards();
...
function handlePresenceTableClicks(event) {
  const dayButton = event.target.closest(".presence-day-btn");
  if (dayButton) {
    if (dayButton.dataset.skipClick === "1") {
      dayButton.dataset.skipClick = "";
      return;
    }
    const nextValue = getNextPresenceStatus(dayButton.dataset.status || "");
    applyPresenceButtonValue(dayButton, nextValue);
    return;
  }
...
function getPresenceRowStatuses(row) {
  if (!row) return [];
  return [...row.querySelectorAll(".presence-day-btn")].map(button => ({
    dayKey: button.dataset.day || "",
    status: button.dataset.status || ""
  }));
}

function buildPresenceSummaryFromStatuses(statuses = [], monthKey = getPresenceMonthKey()) {
  let present = 0;
  let justified = 0;
  let unjustified = 0;
  let launched = 0;

  statuses.forEach(({ dayKey, status }) => {
    if (!/^\d{2}$/.test(dayKey) || isWeekendPresenceDay(monthKey, dayKey)) return;
    if (!["P", "FJ", "FN"].includes(status)) return;
    launched += 1;
    if (status === "P") present += 1;
    if (status === "FJ") justified += 1;
    if (status === "FN") unjustified += 1;
  ...
  const rate = launched ? (present / launched) * 100 : null;
  return { present, justified, unjustified, launched, rate };
}

function refreshPresenceRowSummary(row) {
  if (!row) return;
  const summary = buildPresenceSummaryFromStatuses(getPresenceRowStatuses(row), getPresenceMonthKey());
  const presentCell = row.querySelector("[data-summary='present']");
  const justifiedCell = row.querySelector("[data-summary='justified']");
  const unjustifiedCell = row.querySelector("[data-summary='unjustified']");
  const rateCell = row.querySelector("[data-summary='rate']");

  if (presentCell) presentCell.innerHTML = `<strong>${summary.present}</strong>`;
  if (justifiedCell) justifiedCell.textContent = summary.justified;
  if (unjustifiedCell) unjustifiedCell.textContent = summary.unjustified;
  if (rateCell) rateCell.innerHTML = getAttendanceRateBadge(summary.rate);
}

function refreshPresenceSummaryCards() {
  if (!els.presenceSummaryGrid || !els.presenceTableBody) return;
  const rows = [...els.presenceTableBody.querySelectorAll("tr[data-person-id]")];
  const people = rows.length;
  if (!people) return;

  const summaries = rows.map(row => buildPresenceSummaryFromStatuses(getPresenceRowStatuses(row), getPresenceMonthKey()));
  const launchedPeople = summaries.filter(item => item.launched > 0);
  const averageRate = launchedPeople.length
    ? launchedPeople.reduce((sum, item) => sum + (item.rate || 0), 0) / launchedPeople.length
    : 0;
  const totalPresent = summaries.reduce((sum, item) => sum + item.present, 0);
  const totalJustified = summaries.reduce((sum, item) => sum + item.justified, 0);
  const totalUnjustified = summaries.reduce((sum, item) => sum + item.unjustified, 0);

  els.presenceSummaryGrid.innerHTML = `
    ${statCard("Funcionários / Professores", people)}
    ${statCard("Pessoas com lançamentos", launchedPeople.length)}
    ${statCard("Presenças registradas", totalPresent)}
    ${statCard("Faltas registradas", totalJustified + totalUnjustified)}
    ${statCard("Média de frequência", launchedPeople.length ? formatPercent(averageRate) : "0%")}
...
function openPresenceNoteEditor(personId) {
  ...
  const row = [...els.presenceTableBody.querySelectorAll("tr[data-person-id]")]
...
async function saveMonthlyPresence() {
  ...
  for (const row of rows) {
    ...
    row.querySelectorAll(".presence-day-btn").forEach(button => {
      const dayKey = button.dataset.day || "";
      const value = button.dataset.status || "";
      if (/^\d{2}$/.test(dayKey) && ["P", "FJ", "FN"].includes(value)) {
  ...
  addAudit("Atualização", "Presença", monthLabel, "Calendário diário de presença de funcionários e professores atualizado");
  await saveData();
  renderPresencePanel();
  renderActivityList();
  renderAuditTable();
