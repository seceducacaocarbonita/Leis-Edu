...
function mapElements() {
  [
    ...
    "presenceNoteTitle", "presenceNoteTextarea", "clearPresenceNoteBtn", "savePresenceNoteBtn",
...
function getAdminDataRequirementsForTab(tab = "dashboard") {
  if (!adminLogged) {
    return { ui: false, audit: false, institutional: false };
  }

  const requirements = { ui: false, audit: false, institutional: false };
...
function applySettingsToUI() {
  document.title = "Portal de Gestão Educacional";
  setText(els.orgNameTop, settingsData.orgName);
  setText(els.portalLabelTop, "Secretaria Municipal de Educação");
  setText(els.portalTitleHero, "Portal de Gestão Educacional");
  setText(els.portalSubtitleHero, "Plataforma institucional para consulta, organização e acompanhamento de alunos, funcionários, documentos, leis e indicadores da rede educacional.");
  setText(els.portalTitleFooter, "Portal de Gestão Educacional");
  setText(els.footerOrgText, settingsData.footerText);
  setText(els.sidebarPortalName, "Portal de Gestão Educacional");
  setText(els.sidebarOrgName, settingsData.orgName);
  setText(els.mastheadOrgName, settingsData.orgName);
  setText(els.mastheadPortalName, "Portal de Gestão Educacional");
}

function fillSettingsForm() {
  if (!els.portalTitleInput) return;
  setValue(els.portalTitleInput, "Portal de Gestão Educacional");
...
async function handleSaveSettings(event) {
  ...
  if (els.portalTitleInput) { els.portalTitleInput.value = "Portal de Gestão Educacional"; settingsData.portalTitle = "Portal de Gestão Educacional"; }
  ...
  settingsData.footerLabel = "Secretaria Municipal de educação de Carbonita";
...
function getStudentSupportBadge(record = {}) {
  ...
  return `<span class="student-support-badge" title="Professor de Apoio: ${escapeHtml(supportTeacher)}">Professor de Apoio: ${escapeHtml(supportTeacher)}</span>`;
}
...
function renderInstitutionalTable() {
  ...
  els.institutionRecordsBody.innerHTML = rows.length ? rows.map(record => {
    ...
    <tr class="${supportTeacher ? "student-has-support" : ""}">
      <td><strong>${escapeHtml(record.name)}</strong>${getStudentSupportBadge(record)}</td>
      ...
      <td class="institution-actions-cell">
        <div class="row-actions institution-row-actions" style="display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px;width:168px;max-width:168px;margin:0 auto;align-items:stretch;justify-content:center;">
          <button class="btn btn-light institution-action-btn" style="width:100%;min-width:0;min-height:38px;padding:8px 10px;" data-action="view-person" data-id="${record.id}">Ficha</button>
          <button class="btn btn-light institution-action-btn" style="width:100%;min-width:0;min-height:38px;padding:8px 10px;" data-action="edit-person" data-id="${record.id}">Editar</button>
          <button class="btn btn-light institution-action-btn" style="width:100%;min-width:0;min-height:38px;padding:8px 10px;" data-action="print-person" data-id="${record.id}">Imprimir</button>
          <button class="btn btn-danger institution-action-btn" style="width:100%;min-width:0;min-height:38px;padding:8px 10px;" data-action="delete-person" data-id="${record.id}">Excluir</button>
        </div>
      </td>
    </tr>
  `;
...
function savePresenceNoteEditor() {
  ...
  showToast(note ? "Observação atualizada." : "Observação removida.");
}
