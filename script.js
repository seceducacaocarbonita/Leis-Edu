...
async function handleAdminLogin(event) {
  ...
  try {
    ...
    const normalizedEmail = session?.user?.email || data?.user?.email || email;
    clearLoginAttemptState(normalizedEmail);

    // Não depende apenas do onAuthStateChange.
    // Em alguns navegadores/hospedagens o evento demora ou só é refletido ao atualizar a página.
    // Aqui a sessão retornada pelo Supabase já abre o painel imediatamente.
    await completeAuthTransition("SIGNED_IN", session);
  } catch (error) {
...
function getAuthSessionKey(session = null) {
  if (!session) return "";
  const userKey = session?.user?.id || session?.user?.email || "";
  const tokenKey = session?.access_token ? String(session.access_token).slice(0, 28) : "";
  return `${userKey}:${tokenKey}`;
}

async function completeAuthTransition(event, session, options = {}) {
  const isLoginEvent = event === "SIGNED_IN";
  const isLogoutEvent = event === "SIGNED_OUT";
  const submitBtn = els.adminLoginForm?.querySelector('button[type="submit"]');
  const sessionKey = getAuthSessionKey(session);

  if (isLoginEvent && sessionKey && lastHandledSignInKey === sessionKey && !options.force) {
    if (submitBtn) {
      submitBtn.disabled = false;
      submitBtn.textContent = "Entrar";
    }
  ...
  if (isLoginEvent && adminLogged) {
    lastHandledSignInKey = sessionKey || `${adminUserEmail}:${Date.now()}`;
...
function bindAuthListener() {
  supabase.auth.onAuthStateChange(async (event, session) => {
    ...
    // Ignorar evita dupla execução e race conditions na inicialização.
    if (event === "INITIAL_SESSION") return;

    try {
      await completeAuthTransition(event, session);
    } catch (error) {
      ...
      showToast("Não foi possível abrir o painel automaticamente. Tente novamente.");
    }
  });
}
